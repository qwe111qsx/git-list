...
| Area | What you get |
|------|----------------|
| **Commits** | Paginated history from `HEAD`, expandable file list, open diff per file. Section-level refresh. **Inline** second control (next to refresh): **More commit list actions…** → **Filter by author…** — **whitelist** checked authors only; unchecked authors never appear until you include them again. **All authors checked** removes the filter. After **OK**, this list resets to **page one** and only authors **already seen while loading** this list appear in the picker (expand **Commits** once first if the picker is empty). With a filter on, each page still aims for **`git-list.commitsPageSize`** matching commits by scanning more raw history when needed (bounded by a safety cap in enormous repos; use **Load more** if a page is short). **Context menu:** **Cherry-pick onto current branch…** (`git cherry-pick`), **Create branch from commit…**, **Copy short hash** / **Copy full hash** on any **commit** row. |
| **File History** | Collapsible section (title is fixed English **File History** / **File History (name)**) listing commits for a **pinned source file** (last focused `file://` file in a repo). Switching to a **diff** or other non-file editor does **not** change the list—only focusing another source file, **rename** of that file, **save** of that file, or **refresh** updates it. Uses **`git log --all --follow`** (same paging as **Commits** via `git-list.commitsPageSize`). **⋯** on the section row opens the same **More commit list actions…** / **Filter by author…** whitelist as **Commits**, scoped **only to File History** (independent from **Commits** or a branch’s list). Expand commits to browse files / open diffs like **Commits**. Commit rows share the same **cherry-pick**, **create branch**, and **copy hash** context actions. |
| **Branches** | Paginated local branches (`git branch`), each branch shows paginated `git log` for that ref. Commit rows support **cherry-pick**, **create branch**, and **copy hash**. The **branch** row’s **⋯** (next to refresh for that branch’s commits) opens **More commit list actions…** → **Filter by author…** with a **per-branch** whitelist (separate from **Commits** / **File History**). Branches **not merged into current HEAD** use a **red branch icon** (from `git for-each-ref … --no-merged HEAD`); the label is the plain branch name. **Context:** switch to branch (`git switch`), merge into current branch when unmerged (`git merge`), delete local branch (`git branch -d`). The **Branches section** row’s **⋯** (right of **Refresh branch list**) opens **More branch actions…** → **bulk-delete** locals whose **tip** is older than **6 months** (with progress). |
| **Remotes** | Paginated remote names (`git remote`). Each **remote** row shows **(N)** after the name — number of `refs/remotes/<name>/…` tracking branches (same scope as the list when expanded). Local **branch** rows show the tip **date** on the right instead. Each remote lists remote-tracking branches (`refs/remotes/…`). Same log/diff behavior as **Branches**; each **branch** row’s **⋯** includes **Filter by author…** for that ref’s commit list. Commit rows support **cherry-pick**, **create branch**, and **copy hash**. Unmerged-into-HEAD styling and **merge** when applicable. **Context:** switch — tries `git switch <name>`, then `git switch -c <name> <remote/ref>` if no local branch exists. **Refresh** runs **`git fetch --all --prune`** (section) or **`git fetch <remote> --prune`** (per remote), then reloads the list. Section title is plain English (`Remotes`). |
...
| **Global** | **Refresh** on the view title when a repo exists. **More** (**⋯**) → **Copy** → **branch name** (current checked-out branch; success toast). **More** also offers **show repository status** (`git status` in **Output → Git List**), **abort merge**, **abort rebase**, and **abort cherry-pick** (each with confirmation when the matching Git state exists). Optional auto-refresh when the built-in **Git** extension reports repo or state changes (workspace-scoped). Section panel titles (**Commits**, **File History**, **Branches**, **Remotes**, **Stash**) stay in English. |
| **No Git repository** | If the opened workspace folder is **not** a Git repo, the sidebar shows a **welcome view** with a blue **Initialize Git Repository** button (runs `git init` and registers the repo with the built-in Git extension). The usual **Commits / File History / Branches / Remotes / Stash** sections appear after initialization. The view **Refresh** button is hidden until a repo exists. Switching workspace folders updates the view automatically. |
...
|---------|-------------|----------------|
| **Refresh** | Reload the whole Git List tree. | View title bar (when a Git repo exists) |
| **More…** | Opens the view title menu: **Copy** → **branch name**; **Show repository status**; **Abort merge** / **Abort rebase** / **Abort cherry-pick** when applicable. | View title bar **⋯** (when a Git repo exists) |
| **Copy branch name** | Copies the checked-out branch name (or a detached-HEAD label) and shows a confirmation toast. | **More… → Copy**; Command Palette |
| **Show Repository Status** | Runs `git status`, prints it to **Output → Git List**, and notes merge / rebase / cherry-pick in progress when detected. | **More…**; Command Palette |
| **Abort Merge** | `git merge --abort` after confirmation (only if a merge is in progress and not a cherry-pick). | **More…**; Command Palette |
| **Abort Rebase** | `git rebase --abort` after confirmation (only if a rebase is in progress). | **More…**; Command Palette |
| **Abort Cherry-pick** | `git cherry-pick --abort` after confirmation (only if a cherry-pick is in progress). | **More…**; Command Palette |
| **Initialize Git Repository** | Initializes Git in the first workspace folder and registers it with the built-in Git extension, then shows the normal Git List sections. | Welcome view when the folder is not a Git repo; Command Palette |
...
| **More branch actions…** | Opens a **Quick Pick** on the **Branches** section row: **Delete local branches older than 6 months…** | **Branches** section row (inline **⋯**, right of refresh); Command Palette |
| **Delete local branches older than 6 months…** | Deletes **local** branches whose **tip** committer date is older than 6 months (`git branch -d` each); never deletes the checked-out branch; unmerged branches are left and summarized. Progress in a notification while running. | **More branch actions…**; Command Palette |
...
| **Cherry-pick onto Current Branch…** | Confirms then runs `git cherry-pick <commit>` onto the checked-out branch. Skipped when in detached `HEAD`, when the commit is already `HEAD`, or when it is already an ancestor of `HEAD`. Failures show Git output like other Git List commands. | **Commit** row (context); Command Palette |
...
1. Open a folder or multi-root workspace. If the folder is **not** yet a Git repository, open **Git List** in the Activity Bar and click the blue **Initialize Git Repository** button in the welcome view (or run the command from the **Command Palette**) to initialize Git in that folder. After initialization, the usual **Commits**, **File History**, **Branches**, **Remotes**, and **Stash** sections appear. Changing the workspace folder refreshes this state automatically.
...
3. Click **Git List** in the **Activity Bar**. Use **Refresh** or **More (⋯)** on the view title when a repo is open — **Copy** for the branch name, or **Show repository status** / **Abort merge** / **Abort rebase** / **Abort cherry-pick** when Git is mid-operation.
...
If the **built-in Git** extension shows a warning next to the current branch in the **status bar**, the repo often has **dirty state, conflicts, or an unfinished merge/rebase/cherry-pick**. Run **Show Repository Status** from the view title **More (⋯)** to see `git status` in **Output → Git List**; use **Abort merge**, **Abort rebase**, or **Abort cherry-pick** from the same menu when you intend to cancel that operation.
