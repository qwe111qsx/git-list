...
| **Diff from Git List** | When you open a commit/stash **file diff** from Git List, it opens as a **preview** editor. The diff editor’s **title bar** can show **Open working tree file** — opens the **current checkout** copy of that path on disk (beside the diff) so you can compare with the revision side by side. |
