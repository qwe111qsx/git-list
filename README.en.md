# Git List

A VS Code **sidebar** view that lists **recent commits** and **stashes** for Git repositories in your workspace. Expand a commit or stash to browse changed files; click a file to open a **diff** in the editor. Optional **per-author icon colors** are remembered until you clear them.

For the bilingual readme shown on the Marketplace, see [README.md](https://gitee.com/liuyuanfan6/git-list/blob/main/README.md).

![Extension icon](https://gitee.com/liuyuanfan6/git-list/raw/main/media/icon.png)

## Features

- **Commits** tree with paging (`git-list.commitsPageSize`), file list under each commit, inline diff via the built-in diff editor.
- **Stash** tree with paging (`git-list.stashPageSize`), same diff behavior for stashed changes.
- **Refresh** from the view title; optional auto-refresh when the built-in Git extension opens or closes a repository.
- **Clear saved author icon colors** command when author/account icon colors should reset.

## Requirements

- [Git](https://git-scm.com/) on your `PATH`.
- VS Code **1.85** or newer.
- Recommended: built-in **Git** extension enabled (for repository open/close events).

## Usage

1. Open a folder or workspace that contains Git repositories.
2. Click the **Git List** icon in the **Activity Bar**.
3. Expand **Commits** or **Stash**, then open items or use **Load more** when shown.

## Configuration

| Setting | Default | Description |
|--------|---------|-------------|
| `git-list.commitsPageSize` | `40` | Commits loaded on first expand and per “Load more”. |
| `git-list.stashPageSize` | `40` | Stashes loaded on first expand and per “Load more”. |

## Sponsor

If this extension helps you, you can support further development using the QR codes below (images served from the Gitee `main` branch).

| WeChat | Alipay |
| :----: | :----: |
| <img src="https://gitee.com/liuyuanfan6/git-list/raw/main/media/wechat.png" alt="WeChat QR" width="220" /> | <img src="https://gitee.com/liuyuanfan6/git-list/raw/main/media/alipay.png" alt="Alipay QR" width="220" /> |

## Development

```bash
npm install
npm run compile
```

Press **F5** in VS Code to launch the Extension Development Host.

After editing `media/icon.svg`, regenerate the marketplace icon:

```bash
npm run render-icon
```

## Publishing to the Visual Studio Marketplace

See [marketplace.zh-CN.md](https://gitee.com/liuyuanfan6/git-list/blob/main/docs/marketplace.zh-CN.md) (Chinese) for publisher registration, Personal Access Token, and `vsce` steps.

```bash
npm run package
```

Produces `git-list-<version>.vsix` (version from `package.json`). Use plain English **`displayName`** and **`description`** in `package.json` for the Marketplace listing; VS Code UI strings for views and commands still come from `package.nls*.json`.

## License

MIT — see [LICENSE](https://gitee.com/liuyuanfan6/git-list/blob/main/LICENSE).
