...
export class GitListTreeProvider implements vscode.TreeDataProvider<GitListTreeItem> {
  ...
  private commitLimit = PAGE_SIZE;
  private stashLimit = PAGE_SIZE;
...
async function loadCommits(repo: string, displayLimit: number): Promise<GitListTreeItem[]> {
  try {
    const fetchCount = String(displayLimit + 1);
    const { stdout } = await execFileAsync(
      "git",
      ["log", "--pretty=format:%h%x09%s", "-n", fetchCount],
    ...
    if (hasMore) {
      items.push(
        new GitListTreeItem("loadMoreCommits", "Load more…", vscode.TreeItemCollapsibleState.None)
      );
    }
    return items;
  } catch {
...
async function loadStash(repo: string, displayLimit: number): Promise<GitListTreeItem[]> {
  try {
    ...
    if (hasMore) {
      items.push(
        new GitListTreeItem("loadMoreStash", "Load more…", vscode.TreeItemCollapsibleState.None)
      );
    }
    return items;
  } catch {
